...
void convert_path_to_absolute(char *absolute_path, const char *cwd,
                              const char *path) {
  strcpy(absolute_path, cwd);  // NOLINT (strcpy is unsafe)
  strcat(absolute_path, path); // NOLINT (strcat is unsafe)
}

void process_input_file(const char *input_file, const char *output_file) {
  EXIT_IF(input_file == NULL || output_file == NULL,
