...
void check_constness() {
    ...
    if (s.size() != 5)
        fail("incorrect size");
    auto first = s.begin();
    Set<int>::const_iterator last = s.end();
    if (*first != -4 || *(--last) != 7)
...
/* check if class correctly works with const_iterators */
void check_const_iterators() {
    std::cerr << "check const_iterators... ";
    Set<std::pair<int, int> > s{{-3, 5}, {5, 5}, {-4, 1}, {-4, 4}, {0, 1}, {3, 0}};
    if (s.begin()->second != 1 || (++s.begin())->first != -4)
        fail("wrong begin()");
    Set<std::pair<int, int> >::const_iterator cur = s.end();
    Set<int> small{1};
    ...
    cur--;
    if (begin == cur)
        fail("wrong == for const_iterators");
    while (begin != cur) {
        ++begin;
        --cur;
    }
